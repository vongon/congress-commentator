...
exports.getCampaignExpenditures = getCampaignExpenditures = (cb) => {
	const options = {
		method: 'GET',
		url: `https://api.open.fec.gov/v1/schedules/schedule_b/?api_key=` + config.fec.api_key + `&sort_hide_null=false&sort_nulls_last=true&sort=-disbursement_date&per_page=2&committee_id=` + config.fec.committee_id + `&two_year_transaction_period=2018`,
		json: true
	}
